...
public class Cache {
	private static final Character STAND_BY = new Character('?');
	private static final Character EMPTY = new Character(' ');

	public int leastInterval(char[] tasks, int n) {
		Map<Character, Integer> taskmap = new HashMap<Character, Integer>();
		Queue<Map.Entry<Character, Integer>> queue = new PriorityQueue<Map.Entry<Character,Integer>>(
		...
		System.out.println(new Gson().toJson(taskmap));
		for(Entry<Character, Integer> entry:taskmap.entrySet()) {
			queue.add(entry);
		}
		System.out.println(new Gson().toJson(queue));
		
		List<Character> timeline = new ArrayList<Character>();
		LinkedList<Character> unavailableTask = new LinkedList<Character>();
		
//		Character first = getNext(taskmap, unavailableTask);
		Entry<Character, Integer> firstEntry = getNext(queue, unavailableTask);
		Character first = firstEntry.getKey();
		timeline.add(first);
		for (int i = 0; i < n - 1; i++) {
			unavailableTask.offer(EMPTY);
		}
		unavailableTask.offer(first);
		int countFirst = taskmap.get(first);
		if (countFirst == 1) {
			taskmap.remove(first);
			queue.remove(firstEntry);
		} else {
			taskmap.put(first, taskmap.get(first) - 1);
		}
		
		
		while (!taskmap.isEmpty()) {
			// 找下一个任务
//			Character nextTask = getNext(taskmap, unavailableTask);
			Entry<Character, Integer> entry = getNext(queue, unavailableTask);

			if (entry == null) {
				// 没找到
				unavailableTask.poll();
				unavailableTask.offer(STAND_BY);
				timeline.add(STAND_BY);
			} else {
				// 找到了
				Character nextTask = entry.getKey();
				unavailableTask.poll();
				unavailableTask.offer(nextTask);
				timeline.add(nextTask);

				int count = taskmap.get(nextTask);
				if (count == 1) {
					taskmap.remove(nextTask);
					queue.remove(entry);
				} else {
					taskmap.put(nextTask, taskmap.get(nextTask) - 1);
					queue.remove(entry);
					queue.offer(entry);
				}
			}
...
//			System.out.println(new Gson().toJson(taskmap));
	...
	public <T> Entry<T, Integer> getNext(Queue<Entry<T, Integer>> queue,List<T> list) {
		Iterator<Entry<T, Integer>> iterator = queue.iterator();
		while(iterator.hasNext()) {
	...
	public static void main(String[] args) {
		...
		System.out.println(new Cache().leastInterval(tasks, 1));
